...
import {getEthPriceNow, getEthPriceHistorical} from 'get-eth-price'
...
import { useSingleAddToken } from '../../../hooks/useMint'
...
const CssTextField = withStyles({
  ...
  const InstantSale = () => (
    <div className="mt-2" id="instant-sale">
        <div className="row white col-md-11 left label-create" >
            <b>Instant sale price</b>
        </div>

        <div className="row white" >
          <Switch
            checked={state.checkedB}
            onChange={instantSaleHandleChange}
            color="primary"
            name="checkedB"
            inputProps={{ 'aria-label': 'primary checkbox' }}
          />
        </div>
        <div className="label-description mt--1" >
  ...
  const instantClass =   state.checkedA && state.checkedB ? class1 : class2
  ...
  const instantSaleHandleChange = (event) => {
    ...
    if(!state1)
     document.getElementById('unlockPurchase').classList.add('mt-2')
    else
     document.getElementById('unlockPurchase').classList.remove('mt-2')
  ...
  const { onAddToken } = useSingleAddToken()
  ...
  return (
    <div className="container create-collectible center">
          ...
          <div className="row text-left white col-md-12 mb-2">
            ...
            <Button variant='contained' id='btn-add-token' className='primary' onClick={onAddToken}>Add Single Token</Button>
          </div>
